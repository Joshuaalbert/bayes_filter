...
def test_py_function_optimise_hyperparams(tf_session):
    datapack = make_example_datapack(5,2,2,gain_noise=0.,index_n=2,clobber=True,obs_type='DDTEC',name=os.path.join(TEST_FOLDER,"test_hp_opt_data.h5"))
    ...
    with tf_session.graph.as_default():
        ...
        tf_session.run(init)
        learned_hyperparams = KernelHyperparameterSolveCallback(resolution=3, obs_type='DTEC')(X, mean_ddtec, var_ddtec,0.1, 15., 250., 100., 50.)
        res = tf_session.run(learned_hyperparams)
        print(res)

        tf_session.run(init)
        learned_hyperparams = KernelHyperparameterSolveCallback(resolution=3, obs_type='DDTEC')(X, mean_ddtec, var_ddtec,0.1, 15., 250., 100., 50.)
        res = tf_session.run(learned_hyperparams)
        print(res)