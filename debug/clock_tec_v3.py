...
from bayes_filter.misc import maybe_create_posterior_solsets, get_screen_directions, safe_cholesky, lock_print
import pylab as plt
...
#         with tf.control_dependencies([lock_print([self.phase_dd], 'Solving tec with ref dir', self.ref_dir)]):
#             # Nd*Na, Nt
...
class TecSolve(object):
    ...
    def run(self, parallel_iterations=10):
        ...
        with tf.control_dependencies([lock_print([self.phase_dd], 'Solving tec with ref dir', self.ref_dir)]):
            # Nd*Na, Nt
            tec_mean, tec_uncert = tf.map_fn(self.solve_all_time, grid, parallel_iterations=parallel_iterations,
...
class ResidualSmoothLoss(object):
    def __init__(self, phase_res, freqs):
        ...
        # lstsq mean
        # Nf, 3
        lhs = tf.stack([tf.math.square(self.freqs), self.freqs, tf.ones_like(self.freqs)], axis=1)
        # 3, Nt
        coeffs = tf.linalg.lstsq(lhs, phase_res, fast=False)
        # Nf, Nt
    ...
    def loss_func(self, params):
        ...
        marginal_log = tf.reduce_sum(maha + com[:, None], axis=1) + tfp.distributions.Normal(
            ...
            phase_noise) + tfp.distributions.Normal(loc=tf.constant(0., float_type),
                                                    scale=tf.constant(2.5, float_type)).log_prob(freq_lengthscale)
        return tf.math.negative(marginal_log)
...
if __name__ == '__main__':
    ...
    input_datapack = '/net/lofar1/data1/albert/imaging/data/P126+65_compact_raw/P126+65_full_compact_raw_v7.h5'
