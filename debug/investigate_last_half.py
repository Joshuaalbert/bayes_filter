    ...
    with tf.Session(graph=tf.Graph()) as sess:
        ...
        for t in range(0, Nt, block_size):
            ...
            for a in range(Na_screen):
                ...
                plot_vornoi_map(points, ystar[0, :, a], norm=plt.Normalize(-60., 60.), cmap=plt.cm.gist_rainbow)
                if a in keep:
                    ka = keep.index(a)
                    detection_mask = detection.reshape((stop - start, Nd, Na))[0, :, ka]
                    plt.scatter(ra, dec, s=30., c=tec[start, :, ka], cmap=plt.cm.gist_rainbow, vmin=-60., vmax=60.,
                                edgecolors='black')
                    plt.scatter(ra[detection_mask], dec[detection_mask], s=100., c=tec[start, detection_mask, a],
                                cmap=plt.cm.gist_rainbow, vmin=-60., vmax=60., edgecolors='black')
                plt.title(ants[a])
