...
class PrepareH5ParmFeeds(object):
    def __init__(self, tf_session, datapack, solset, srl_fits, index_feed_N=2, output_solset_base='dr2_saem',
                 remake_posterior=False, max_screen_dirs=None, ref_dir=None,
                 ant_sel=None, time_sel=None, dir_sel=None, freq_sel=None, pol_sel=None):
        logging.info("Preparing feeds from an h5parm: {}".format(datapack))

        if isinstance(datapack, str):
            datapack = DataPack(datapack, readonly=True)
        self.datapack = datapack

        logging.info("Using output solset base: {}".format(output_solset_base))
        self.output_solset = self.output_solset(output_solset_base)
        self.screen_output_solset = self.output_screen_solset(output_solset_base)
        self.solset = solset
        ...
        with datapack:
            ...
            screen_directions = get_screen_directions(srl_fits, max_N=max_screen_dirs)
            self._maybe_create_posterior_solsets(solset, screen_directions, remake_posterior_solsets=remake_posterior)
            ...
            if ref_dir is None:
                ref_dir = np.mean(
                    np.stack([directions.ra.to(angle_type).value, directions.dec.to(angle_type).value], axis=1), axis=0)
            ...
            ###
            # get screen
            datapack.switch_solset(self.screen_output_solset)
            ...
            # Nd, 2
            self.screen_Xd = np.stack(
                [screen_directions.ra.to(angle_type).value, screen_directions.dec.to(angle_type).value], axis=1)
            # Na, 3
            self.screen_Xa = antennas.cartesian.xyz.to(dist_type).value.T

            # sampling the screen and data means slice size needs to take into account bothe
            ...
            with tf_session.graph.as_default():
                self.index_feed = IndexFeed(index_feed_N)
                # tf_session.run(index_feed.index.get_init())
                time_feed = TimeFeed(self.index_feed,
                                     tf.convert_to_tensor(self.Xt, dtype=float_type))
                self.coord_feed = CoordinateFeed(time_feed,
                                                 tf.convert_to_tensor(self.Xd, dtype=float_type),
                ...
                self.star_coord_feed = CoordinateFeed(time_feed,
                                                      tf.convert_to_tensor(self.screen_Xd, dtype=float_type),
                ...
                self.data_feed = DataFeed(self.index_feed, self.Y_real_pl, self.Y_imag_pl, event_size=1)
                self.freqs = tf.convert_to_tensor(freqs, float_type)
                self.y_sigma = tf.convert_to_tensor(
                    np.square(np.diff(self.Y_real, axis=0)).mean() + np.square(np.diff(self.Y_imag, axis=0)).mean(),
                    float_type)
    ...
    def store_results(self, results, output_folder):
        ...
        with self.datapack:
            ...
            self.datapack.select(ant=self.ant_sel, time=self.time_sel, dir=self.dir_sel, freq=self.freq_sel,
                                 pol=self.pol_sel)
            # Nt, Nd, Na, Nf
    ...
    def _maybe_create_posterior_solsets(self, solset, screen_directions,
                                        remake_posterior_solsets=False):
        with self.datapack:
            if remake_posterior_solsets:
                self.datapack.delete_solset(self.output_solset)
                self.datapack.delete_solset(self.screen_output_solset)
            if not self.datapack.is_solset(self.output_solset) or not self.datapack.is_solset(
                    self.screen_output_solset):
                ...
                self.datapack.add_freq_dep_tab('phase', times.mjd * 86400., freqs=freqs, pols=pol_labels)
                self.datapack.add_freq_dep_tab('amplitude', times.mjd * 86400., freqs=freqs, pols=pol_labels)
                self.datapack.switch_solset(self.screen_output_solset,
                                            array_file=DataPack.lofar_array,
...
if __name__ == '__main__':
    ...
    with sess:
        data_obj = PrepareH5ParmFeeds(sess,
                                      '/home/albert/git/bayes_tec/scripts/data/P126+65_compact_full_raw.h5',
                                      'sol000',
                                      '/home/albert/git/bayes_tec/scripts/data/image.pybdsm.srl.fits',
                                      max_screen_dirs=None,
                                      time_sel=slice(0,30,1),
                                      dir_sel=slice(0,None,1),
                                      ant_sel=slice(48,55,1),
                                      pol_sel=slice(0,1,1),
                                      index_feed_N=1)
        ...
        filtered_res, inits = free_transition.filter_step(
            ...
            init_kern_hyperparams={'y_sigma':0.07,'variance':0.5e-4,'timescale':45.,'lengthscales':15., 'a':200., 'b':60.},
        ...
        while cont:
            ...
            logging.info("Step took {} seconds".format(default_timer() - t0))
